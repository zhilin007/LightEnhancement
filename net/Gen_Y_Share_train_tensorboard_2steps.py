...
def train_y(net,loader_train,loader_test,loader_eval_train,optim_y,criterion,steps):
	...
	for step in range(start_step+1,steps+1):
		...
		print(f'\rtrain_Y loss : {loss_y.item():.5f}| step :{step}/{opt.steps}|lr :{lr :.7f} |esti_time :{esti_time/60:.1f}h ',end='',flush=True)
...
def train_out(net,loader_train,loader_test,loader_eval_train,optim_y,optim_net,criterion,steps):
	losses=[]
	start_step=0
	max_ssim=0
	max_psnr=0
	...
	for step in range(start_step+1,steps+1):
		...
		esti_time=((time.time()-start_time)/60)/(step-start_step)
		esti_time=(opt.steps-step)*esti_time
		print(f'\rtrain_y loss : {loss_y.item():.5f}|train_out loss : {loss_out.item():.5f}| step :{step}/{steps}|lr :{lr :.7f} |esti_time :{esti_time/60:.1f}h ',end='',flush=True)
		if step % opt.eval_step ==0 :
			with torch.no_grad():
...
if __name__ == "__main__":
	...
	optimizer_y = RAdam(params=[
		{'params': net.encoder.parameters()},
		{'params': net.spp.parameters()},
		{'params': net.decoder2.parameters()},
		{'params': net.post2.parameters()},],lr=opt.lr, betas = (0.9, 0.999), eps=1e-08)
	optimizer_y.zero_grad()
	optimizer_net=RAdam(params=[
		{'params': net.encoder.parameters()},
		{'params': net.spp.parameters()},
		{'params': net.decoder.parameters()},
		{'params': net.post.parameters()},
		{'params': net.filter.parameters()},
		{'params': net.guided_map.parameters()},],lr=opt.lr, betas = (0.9, 0.999), eps=1e-08)
	...
	train_out(net,loader_train,loader_test,loader_eval_train,optimizer_y,optimizer_net,criterion,opt.steps)
