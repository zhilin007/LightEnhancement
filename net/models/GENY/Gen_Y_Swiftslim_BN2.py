...
class ResNet18(nn.Module):
	def __init__(self,color=4,layers=[2,2,2,2],features=[16,32,64,64],norm=False):
		...
		self.conv1 = nn.Conv2d(color, features[0], kernel_size=7, stride=2, padding=3,#7x7
							bias=True)
...
class Gen_Y(nn.Module):
	def __init__(self,incolor=4,outcolor=3,features=[16,32,64,64],norm=False,scale_factor=0.25):
		super(Gen_Y,self).__init__()
		self.sppdim=64
		self.encoder=ResNet18(incolor,features=features,norm=norm)
	...
	def forward(self,x,ill):
		...
		return Y
class Gen_O(nn.Module):
	def __init__(self,incolor=4,outcolor=3,features=[16,32,64,64],norm=False,scale_factor=0.25):
		super(Gen_O,self).__init__()
		self.sppdim=64
		self.encoder=ResNet18(incolor,features=features,norm=norm)
		self.spp=SpatialPyramidPooling(features[-1],out_size=self.sppdim,norm=norm)
...
if __name__ == "__main__":
	...
	net=Gen_Y_Swiftslim_BN2(norm=True)
